    ...
    <div class="table-wrapper">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $t->no_pesanan }}</td>
                    <td>{{ $t->customer?->nama ?? 'Customer Terhapus' }}</td>
                    <td>{{ $t->created_at->format('d F Y') }}</td>
                    <td style="color:var(--primary);font-weight:600;">Rp {{ number_format($t->total_harga, 0, ',', '.') }}</td>
                    <td>
                        @if($t->bukti_bayar)
                            <a href="{{ asset('uploads/bukti/'.$t->bukti_bayar) }}" target="_blank" style="color:var(--primary);">Lihat Bukti</a>
                        @else
                    ...
                    <td>
                        <button class="btn btn-edit"
                            onclick="openDetailModal(
                                {{ $t->id }},
                                '{{ $t->customer?->nama ?? 'Customer Terhapus' }}',
                                '{{ $t->stokIkan?->jenis_ikan ?? 'Produk Terhapus' }}',
                                '{{ $t->ukuran }}',
                                {{ $t->jumlah }},
                                {{ $t->total_harga }},
                                '{{ $t->status }}',
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                    <input type="text" id="d-bukti" readonly style="flex:1;">
                    <a id="btn-lihat-bukti-modal" href="#" target="_blank"
                       style="display:none;text-decoration:none;padding:10px 14px;border-radius:10px;font-size:0.8rem;font-weight:600;white-space:nowrap;background:linear-gradient(135deg,var(--primary),var(--accent));color:#fff;align-items:center;justify-content:center;height:38px;">
                        Lihat
                    </a>
                </div>
            </div>
...
function openDetailModal(id, customer, produk, ukuran, jumlah, harga, status, alamat, bukti, statusPembayaran) {
    ...
    const btnLihatBukti = document.getElementById('btn-lihat-bukti-modal');
    if (bukti) {
        btnLihatBukti.href = '/uploads/bukti/' + bukti;
        btnLihatBukti.style.display = 'inline-flex';
    } else {
        btnLihatBukti.href = '#';
        btnLihatBukti.style.display = 'none';
    }
